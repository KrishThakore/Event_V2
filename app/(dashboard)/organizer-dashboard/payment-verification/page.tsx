...
import PaymentVerificationClient from '../../admin-dashboard/payment-verification/PaymentVerificationClient';

export const revalidate = 0;

export default async function OrganizerPaymentVerificationPage({ 
  searchParams 
}: { 
  ...
  const [registrations, totalCount, pendingCount, rejectedCount] = await Promise.all([
    ...
    prisma.registrations.count({ 
      where: { 
        ...
        payment_method: 'qfix',
        event: { assigned_organizer: userId }
      } 
    }),
    prisma.registrations.count({ 
      where: { 
