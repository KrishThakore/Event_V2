...
async function saveOrganizerScannerAssignments(formData: FormData) {
  'use server';
  ...
  if (!scannerId) {
    redirect('/organizer-dashboard/scanner-access');
  }
  ...
  const authorizedEventIds = authorizedEvents.map((event) => event.id);

  const requestedEventIds = Array.from(
    new Set(formData.getAll('eventIds').map((value) => String(value).trim()).filter(Boolean))
  );
  const scopedEventIds = requestedEventIds.filter((eventId) => authorizedEventIds.includes(eventId));
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (scopedEventIds.length > 0) {
      await tx.event_scanner_assignments.createMany({
        data: scopedEventIds.map((eventId) => ({
          event_id: eventId,
          scanner_id: scannerId,
    ...
    await tx.organizer_logs.create({
      data: {
        ...
        details: {
          ...
          event_ids: scopedEventIds,
          managed_scope_event_ids: authorizedEventIds,
  ...
  revalidatePath('/organizer-dashboard/scanner-access');
  redirect(`/organizer-dashboard/scanner-access?scanner=${encodeURIComponent(scannerId)}&saved=1`);
}

export default async function OrganizerScannerAccessPage({
  searchParams,
}: {
  ...
  const selectedScannerId =
    searchParams.scanner && scanners.some((scanner) => scanner.id === searchParams.scanner)
  ...
  const assignments =
    selectedScannerId && events.length > 0
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-gray-600">
          Assign scanner accounts to the events you manage so they can scan attendance without organizer permissions.
        </p>
      </div>
      ...
      <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        {searchParams.saved === '1' && selectedScanner && (
          <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800">
            Saved scanner access for {selectedScanner.full_name}.
          </div>
        )}
      </div>

      {selectedScanner ? (
        <form action={saveOrganizerScannerAssignments} className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <div className="flex flex-col gap-2 border-b border-gray-100 pb-5 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="rounded-2xl bg-purple-50 px-4 py-3 text-sm font-medium text-purple-800">
              This only manages scanner access for the events assigned to you.
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4">
            {events.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 px-6 py-10 text-center text-sm text-gray-500">
                You do not have any events available to assign.
              </div>
            ) : (
              events.map((event) => (
                <label
        ...
        <div className="rounded-3xl border border-dashed border-gray-200 bg-white p-10 text-center text-sm text-gray-500 shadow-sm">
          Promote a user to the <span className="font-bold text-gray-900">scanner</span> role first, then assign them to your events here.
        </div>
      )}
    </div>
  );
