...
export async function POST(req: NextRequest) {
  try {
    ...
    // Get events authorized for this organizer
    const authorizedEvents = await prisma.events.findMany({
      where: user.role === 'admin' ? {} : {
        OR: [{ created_by: user.id }, { assigned_organizer: user.id }]
      },
      select: { id: true }
    });
    const authorizedEventIds = authorizedEvents.map((e: any) => e.id);
    ...
    if (exportType === 'registrations') {
      const registrations = await prisma.registrations.findMany({
        where: { 
          event_id: { in: authorizedEventIds },
          status: { not: 'PENDING' }
        },
        include: {
          user: true,
      ...
      const attendance = await prisma.attendance.findMany({
        where: { registration: { event_id: { in: authorizedEventIds } } },
        include: {
          scanned_by_profile: {
            select: {
              full_name: true,
      ...
      const headers = ['Payment ID', 'User Name', 'Event Title', 'Amount', 'Status', 'Razorpay Order ID', 'Registration ID', 'Created At'];
    ...
    } else if (exportType === 'event_detailed' && eventId) {
      if (!authorizedEventIds.includes(eventId)) {
      ...
      // Dynamic headers for custom form fields
      const customFieldLabels = Array.from(new Set(
        registrations.flatMap((r: any) => r.responses.map((resp: any) => resp.field?.label).filter(Boolean))
      )).sort() as string[];

      const headers = ['Registration ID', 'User Name', 'User Email', 'Phone', 'University', ...customFieldLabels, 'Event Title', 'Status', 'Paid Amount', 'Entry Code', 'Attendance Status', 'Check-in Time', 'Scanned By Name', 'Scanned By Email', 'Scanned By Role', 'Razorpay Payment ID', 'Razorpay Order ID', 'Created At'];
      const rows = registrations.map((reg: any) => {
        const attendance = reg.attendance;
        const responseMap = new Map();
        reg.responses.forEach((resp: any) => {
          if (resp.field?.label) {
            responseMap.set(resp.field.label, resp.value);
          }
        });

        const rowData: any = {
          'Registration ID': reg.id,
          'User Name': reg.user?.full_name || 'N/A',
          'User Email': reg.user?.email || 'N/A',
          'Phone': reg.phone_number || reg.user?.phone_number || '—',
          'University': reg.university || reg.user?.university || '—',
          'Event Title': reg.event?.title || 'N/A',
          'Status': reg.status,
          ...
          'Entry Code': reg.entry_code || 'N/A',
          'Attendance Status': attendance ? 'CHECKED_IN' : 'NOT_CHECKED_IN',
          'Check-in Time': attendance?.checked_in_at ? formatToIST(attendance.checked_in_at) : 'â€”',
          'Scanned By Name': attendance?.scanned_by_profile?.full_name || 'â€”',
          'Scanned By Email': attendance?.scanned_by_profile?.email || 'â€”',
          'Scanned By Role': attendance?.scanned_by_role || 'â€”',
        };

        customFieldLabels.forEach(label => {
          rowData[label] = responseMap.get(label) || '';
        });

        rowData['Razorpay Payment ID'] = reg.payments?.[0]?.razorpay_payment_id || '—';
        rowData['Razorpay Order ID'] = reg.payments?.[0]?.razorpay_order_id || '—';

        rowData['Created At'] = formatToIST(reg.created_at);
        return rowData;
      });
      csvData = generateCSV(rows, headers);
